...
Written for Windows (the `.bat` launchers); the equivalent `.sh` scripts
exist for Camunda if you're on Linux/Mac, but the Node/JDK pinning steps
below still apply the same way.
...
- `directus/api/.env`: generate fresh `KEY`/`SECRET` UUIDs, set `ADMIN_EMAIL`/`ADMIN_PASSWORD`.
...
## 6. Start everything (order matters)
...
Directus needs a moment to run its first-boot migrations on a fresh
`data.db` (it creates the file itself — nothing to do there). Camunda
auto-deploys everything under `configuration/resources/` on boot, so the
BPMN process and DMN table come up automatically.

## 7. Restore the Directus schema

A fresh `data.db` has Directus's own system tables and your admin user,
...
accounts (hardcoded in `platform/src/users.js`, not env-configured):
...
- **Applicant tab shows no fields / "Applications" table errors** — you skipped step 7; the `license_applications` collection doesn't exist yet on a fresh database.
